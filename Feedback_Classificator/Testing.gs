...
function executeAutoCreationForActiveSheet() { // get the ranges and classification list
...
function checkLastStoredRow(){ // separately stores the last row globally
  const sheetHandler = SpreadsheetApp.getActive().getSheetByName('Form Responses')
  const lastRow = sheetHandler.getLastRow()
  lastRowInMemory = lastRow  
}
